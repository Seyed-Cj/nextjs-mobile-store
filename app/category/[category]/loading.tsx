...
export default function CategoryLoading() {
  return (
    <div
      ...
      aria-label="در حال بارگذاری دسته‌بندی محصولات..."
    >
      {/* 1. Category Header Skeleton */}
      <div className="mb-8 flex flex-col gap-2">
        <div className="h-8 w-48 rounded-xl bg-neutral-200/90" />
        <div className="h-4 w-72 rounded-md bg-neutral-200/60" />
      </div>

      {/* 2. Main Layout: Sidebar Filters + Products Grid */}
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-8 items-start">
        {/* Sidebar Filters (Right in RTL, 3 cols) */}
        <div className="lg:col-span-3 space-y-6 rounded-3xl border border-neutral-200/60 bg-neutral-50 p-5">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex flex-wrap gap-2">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-8 w-14 rounded-lg bg-neutral-200/70" />
              ))}
            </div>
          </div>
        </div>

        {/* Products Column (Left in RTL, 9 cols) */}
