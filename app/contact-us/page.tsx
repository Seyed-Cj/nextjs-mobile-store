...
export default function ContactPage() {
  const breadcrumbItems = [
    { label: contactData.breadcrumb.home, href: "/" },
    { label: contactData.breadcrumb.contact },
  ];
  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <section aria-label="اطلاعات تماس و تصویر فروشگاه">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
            ...
            <div className="lg:col-span-6 flex flex-col justify-center">
              <ContactInfoCards cards={contactData.contactCards} />
            </div>

            {/* Left Column in RTL: Store Image */}
            <div className="lg:col-span-6 min-h-87.5 lg:min-h-0">
              <ContactStoreImage
                src={contactData.storeImage.src}
                alt={contactData.storeImage.alt}
              />
            </div>
          </div>
        </section>
        ...
        {/* 4. Bottom Info Strip */}
        <ContactHoursStrip cards={contactData.bottomInfoCards} />
      </div>
    </main>
  );
}
